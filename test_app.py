...
def test_book_not_found(client):
    response = client.post('/zoek', data={'isbn': '0000000000000'}, follow_redirects=True)
    assert response.status_code == 200
    assert b"niet gevonden" in response.data

def test_invalid_isbn_length_short(client):
    response = client.post('/zoek', data={'isbn': '123'}, follow_redirects=True)
    assert response.status_code == 200
    assert b"Ongeldig ISBN" in response.data

def test_invalid_isbn_length_long(client):
    response = client.post('/zoek', data={'isbn': '1234567890123456'}, follow_redirects=True)
    assert response.status_code == 200
    assert b"Ongeldig ISBN" in response.data
